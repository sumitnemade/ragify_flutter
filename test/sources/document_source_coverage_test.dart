  ...
  group('DocumentSource Coverage Tests', () {
    ...
    group('Cache and Performance Coverage', () {
      ...
      test('covers refresh functionality', () async {
        ...
        expect(
          refreshedChunks.length,
          greaterThanOrEqualTo(initialChunks.length),
        );
      });
    });
    ...
    group('Edge Cases Coverage', () {
      ...
      test('covers maxChunks parameter', () async {
        ...
        for (int i = 0; i < 5; i++) {
          ...
          await file.writeAsString(
            'Content for file $i. This is some test content that should generate chunks.',
          );
        }

        final allChunks = await source.getChunks(query: 'test');
        final limitedChunks = await source.getChunks(
          query: 'test',
          maxChunks: 2,
        );
      ...
      test('covers minRelevance filtering', () async {
        ...
        await testFile.writeAsString(
          'This content has varying relevance to different queries',
        );
        ...
        expect(
          highRelevanceChunks.length,
          greaterThanOrEqualTo(lowRelevanceChunks.length),
        );
      });
    });
